...
def calculate_demographic_data(print_data=True):
    ...
    percentage_bachelors = round(100*(df.education.value_counts()['Bachelors'])/len(df.index), 1)
    ...
    higher_education_rich = round(100*len(df.loc[df.education.isin(degree) & (df.salary == '>50K')])/len(higher_education), 1)
    lower_education_rich = round(100*len(df.loc[~df.education.isin(degree) & (df.salary == '>50K')])/len(lower_education), 1)
    ...
    country_ratio = df.loc[df.salary == '>50K']['native-country'].value_counts()/df['native-country'].value_counts()
